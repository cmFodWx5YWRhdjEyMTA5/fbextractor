...
function extractInfo($credentials, $options) {
	try {
		// $m = new MongoClient();
		// $collection = $m->selectCollection('test', 'bronco');
...
<?php
		try {
			$responsePost = $fb->get('/' . $pageId . '/posts?limit=100');
			...
			do {
				foreach ($postEdge as $post) {
					...
					if ( (isset($options['n']) && $postCount <= $options['n']) ||  !isset($options['n']) ) {
						if (!isset($data["pages"][$pageId]["posts"][$post['id']])) {
							...
							try {
								$responseComment = $fb->get('/' . $post['id'] . '/comments?limit=100');
								...
								do {
									foreach ($commentEdge as $comment) {
										...
										try {
											$responseReplies = $fb->get('/' . $comment['id'] . '/comments?limit=100');
											...
											do {
												foreach ($replyEdge as $reply) {
													...
													$responseReplyLikes = $fb->get('/' . $reply['id'] . '/likes?limit=100');
										...
										try {
											$responseCommentLikes = $fb->get('/' . $comment['id'] . '/likes?limit=100');
							...
							try {
								$responsePostLikes = $fb->get('/' . $post['id'] . '/likes?limit=100');
							...
							try {
								$responsePostShares = $fb->get('/' . $post['id'] . '/sharedposts?limit=100');
				...
				// $collection->insert($data["pages"][$pageId]["posts"][$post['id']]);
			} while ($postEdge = $fb->next($postEdge));
		} catch(Exception $e) {
			output('E: ' . '/' . $pageId . '/posts');
		}
